    ...
    _media(0)
{
    ...
    connect( m_clientSocket, SIGNAL(connected()), SLOT(onConnected()), Qt::UniqueConnection);
    connect( m_clientSocket, SIGNAL(disconnected()), SLOT(onDisconnected()), Qt::UniqueConnection);

    connect( m_clientSocket, SIGNAL(readyRead()), SLOT(onReadyRead()), Qt::UniqueConnection );
    connect( m_clientSocket, SIGNAL(error(QAbstractSocket::SocketError)), SLOT(onConnectionError()), Qt::UniqueConnection );
    connect( m_clientSocket, SIGNAL(stateChanged(QAbstractSocket::SocketState)), SLOT(onStateChange()), Qt::UniqueConnection);
...
//        m_CurrentPlayOrder = m_serverOrder;
//        playNextOrder();
        QElapsedTimer timer;
        timer.start();

        m_clientSocket->connectToHost( hostIP, m_port, QIODevice::ReadWrite );
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    {
        loadSettings();
        Settings *m_ConfigureDig = new Settings(this, m_confUrl, hostIP);
        ...
        if(m_ConfigureDig->exec() == QDialog::Accepted)
        {
            ...
            hostIP = m_ConfigureDig->getHostIP();
            qDebug() << m_confUrl;
...
//    m_clientSocket = static_cast<QTcpSocket*>( sender() );
//    QByteArray* buffer = buffers.value( m_clientSocket );
//    qint32* s = sizes.value( m_clientSocket );
...
void MainWindow::onReadyRead()
{
    ...
    while ( m_clientSocket->bytesAvailable() > 0 ) {
        ...
        buffer.append( m_clientSocket->readAll() );
        ...
        *s = size;
        ...
        m_serverOrder = str.toInt();
        m_CurrentPlayOrder = m_serverOrder;
        qDebug() << "m_serverOrder: -------------------------" << str;
        playNextOrder();
...
{
    qDebug() << "stateChange!: " << m_clientSocket->state();
...
//        m_clientSocket->connectToHost( hostIP, m_port, QIODevice::ReadWrite );
//        m_clientSocket->waitForConnected();
    } else if ( m_clientSocket->state() == QAbstractSocket::ConnectedState ) {

        QByteArray* buffer = buffers.value( m_clientSocket );
        qint32* s = new qint32(0);
        buffers.insert( m_clientSocket, buffer );
        sizes.insert( m_clientSocket, s );
    }
}

void MainWindow::onConnectionError()
{
    qDebug() << "connectionError! " << m_clientSocket->errorString();
}
...
{
    m_clientSocket->connectToHost( hostname, port, QIODevice::ReadWrite );
    m_clientSocket->waitForConnected();
}
...
void MainWindow::sendMessage(QString msgToSend)
{
    ...
    m_clientSocket->write( vDataToSend, vDataToSend.length() );
...
//    QElapsedTimer timer;
//    timer.start();
    while( (b_keyQ ==false) && (m_clientSocket->state() != QAbstractSocket::ConnectedState) ) {
        m_clientSocket->connectToHost( hostIP, m_port, QIODevice::ReadWrite );
        m_clientSocket->waitForConnected();
...
void MainWindow::loadSettings()
{
    ...
    hostIP = settings.value(HOST_IP_ADDRESS, "192.168.1.106" ).toString();
...
void MainWindow::saveSettings()
{
    ...
    settings.setValue(HOST_IP_ADDRESS, hostIP);
